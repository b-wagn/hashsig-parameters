...
def lower_bound_parameter_len(
    log_lifetime: int, num_chains: int, chunk_size: int
) -> int:
    """
...
def lower_bound_hash_len(log_lifetime: int, num_chains: int, chunk_size: int) -> int:
    ...
    """
    min_hash_len_classical = math.ceil(
    ...
    min_hash_len_quantum = math.ceil(
        2
        * (
...
def lower_bound_rand_len_winternitz(log_lifetime: int) -> int:
    ...
    """
    rand_len_classical = math.ceil(
        SECURITY_LEVEL_CLASSICAL + math.log2(5) + log_lifetime + 1
    )
    rand_len_quantum = math.ceil(
        2 * (SECURITY_LEVEL_QUANTUM + math.log2(5) + math.log2(3)) + log_lifetime
    )
...
def lower_bound_message_hash_len_target_sum() -> int:
    ...
    min_msg_hash_len_quantum = math.ceil(
        2 * (SECURITY_LEVEL_QUANTUM + math.log2(5) + 1) + 3
    )
